...
header {visibiimport streamlit as st
...
# ==========================================
# 建議放在這裡！讓使用者一開網頁、隱藏掉不需要的表頭後，就能第一眼看到溫馨提醒
    ...
    if st.session_state.current_member is None:
        ...
        if is_signed:
            ...
            if st.button(f"🟢 若完成【{current_week_display}】請按此簽到", type="primary", use_container_width=True):
                ...
                for m_item in current_year_missing:
                    records_to_add.append((m_item["key"], member_name))
                
                add_batch_records(records_to_add)

                if current_year_missing:
                    st.toast(f"🎉 簽到成功！並已自動為您補齊本年度 {len(current_year_missing)} 週進度！")
                else:
                    st.toast("🎉 簽到成功！")
